...
@Component
public class KafkaListenerDeadLetterMongoTemplate {

    private final MongoTemplate mongoTemplate;

    /**
     * KafkaListenerDeadLetterDocument를 저장합니다.
     * - messageKey가 있는 경우: messageKey로 조회하여 upsert (중복 방지)
     * - messageKey가 없는 경우: 단순 insert
     *
     * @param document 저장할 KafkaListenerDeadLetterDocument
     * @return upsert 결과 (messageKey가 있는 경우에만 의미 있음)
     */
    public UpdateResult upsertKafkaListenerDeadLetter(KafkaListenerDeadLetterDocument document) {
        if (document.getMessageKey() != null && !document.getMessageKey().isEmpty()) {
            ...
            log.debug("Inserted KafkaListenerDeadLetterDocument without messageKey: category={}, isCompleted={}",
     ...
     */
    private UpdateResult upsertByMessageKey(KafkaListenerDeadLetterDocument document) {
        ...
        UpdateResult result = mongoTemplate.upsert(query, update, KafkaListenerDeadLetterDocument.class);

        log.debug("Upserted KafkaListenerDeadLetterDocument: messageKey={}, matched={}, modified={}, upsertedId={}",
    ...
    /**
     * 여러 KafkaListenerDeadLetterDocument를 일괄 저장합니다.
     *
     * @param documents 저장할 KafkaListenerDeadLetterDocument 리스트
     */
    public void upsertAll(Iterable<KafkaListenerDeadLetterDocument> documents) {
        for (KafkaListenerDeadLetterDocument document : documents) {
            upsertKafkaListenerDeadLetter(document);
        }
    }
}